...
Usage:
    python scripts/create_normalization_stats.py \
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Đường dẫn để lưu file normalization_stats.npz. Mặc định: src/normalization_stats.npz",
    )
    parser.add_argument(
        "--sample_size",
        type=int,
    ...
    if args.train_parquet is None:
        train_parquet = str(
            PROJECT_ROOT / "data" / "train_ember_2018_v2_features_label_other.parquet"
        )
    else:
        train_parquet = args.train_parquet
    
    if args.output_path is None:
        output_path = str(PROJECT_ROOT / "src" / "normalization_stats.npz")
    else:
